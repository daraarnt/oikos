...
import { screenCscSiting } from '../../services/poi/cscSitingService.js';
...
export default async function listingsPlugin(fastify) {
  ...
  // A conservative map-data screen for prospective CSC premises. This route is
  // intentionally separate from the listing payload: it can require a handful
  // of throttled/cached Overpass lookups, while opening an ordinary listing must
  // stay fast and must not create location traffic unnecessarily.
  fastify.get('/:listingId/csc-siting', async (request, reply) => {
    const { listingId } = request.params;
    const userId = request.session?.currentUser;
    const listing = listingStorage.getListingById(listingId, userId, isAdminFn(request));
    if (!listing) {
    ...
    return screenCscSiting(listing);
  });
